define(['app'], function(app) {
  ...
  app.factory('yg.services.pdf', [
    function() {

      var pdfFactory = {};

      pdfFactory.parseContainer = function (cnt, e, p, styles) {
        var elements = [];
        var children = e.childNodes;
        if (children.length !== 0) {
          for (var i = 0; i < children.length; i++) {
            p = pdfFactory.parseElement(elements, children[i], p, styles);
          }
        }
      ...
      pdfFactory.computeStyle = function(o, styles) {
      ...
      pdfFactory.createParagraph = function() {
        var p = {text:[]};
        return p;
      };

      pdfFactory.parseElement = function (cnt, e, p, styles) {
        if (!styles) {
          styles = [];
        }
        if (e.getAttribute) {
        ...
        switch (e.nodeName.toLowerCase()) {
          case "#text": {
            ...
            if (styles) {
              pdfFactory.computeStyle(t, styles);
            }
            p.text.push(t);
            break;
          }
          ...
          case "strong": {
            ...
            pdfFactory.parseContainer(cnt, e, p, styles.concat(["font-weight:bold"]));
            break;
          }
          case "u": {
            //styles.push("text-decoration:underline");
            pdfFactory.parseContainer(cnt, e, p, styles.concat(["text-decoration:underline"]));
            break;
          }
          case "i": {
            //styles.push("font-style:italic");
            pdfFactory.parseContainer(cnt, e, p, styles.concat(["font-style:italic"]));
            //styles.pop();
            break;
            //cnt.push({ text: e.innerText, bold: false });
          }
          case "span": {
            pdfFactory.parseContainer(cnt, e, p, styles);
            break;
          }
          case "br": {
            p = pdfFactory.createParagraph();
            cnt.push(p);
            break;
          }
          case "table":
          {
            ...
            pdfFactory.parseContainer(t.table.body, e, p, styles);
          ...
          case "tr": {
            var row = [];
            pdfFactory.parseContainer(row, e, p, styles);
            cnt.push(row);
            break;
          }
          case "td": {
            ...
            pdfFactory.parseContainer(st.stack, e, p, styles);
            cnt.push(st);
            break;
          }
          case "div":
          case "p": {
            ...
            var st = {stack: []};
            st.stack.push(p);
            pdfFactory.computeStyle(st, styles);
            pdfFactory.parseContainer(st.stack, e, p);
      ...
      pdfFactory.parseHtml = function (cnt, htmlText) {
        htmlText = "<div>" + htmlText + "</div>";
        var html = $(htmlText.replace(/\t/g, "").replace(/\n/g, ""));
        var p = pdfFactory.createParagraph();
        for (var i = 0; i < html.length; i++) {
          pdfFactory.parseElement(cnt, html.get(i), p);
        }
      };

      pdfFactory.saveAsPdf = function(reminders) {
        var docDefinition = {
          content: [],
          styles: {
            dateGroup: {
